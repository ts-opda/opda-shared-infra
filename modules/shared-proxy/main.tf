...
locals {
  ...
  endpoint      = "https://${var.external_hostname}.${var.external_domain_name}"
}
...
resource "aws_security_group" "nlb" {
  ...
  description = "Allow external HTTPS to the shared NLB"
  vpc_id      = var.vpc_id
    ...
    Statement = [
      {
        ...
        Resource = [
          aws_ssm_parameter.server_tls_certificate.arn,
          aws_ssm_parameter.server_tls_key.arn,
      ...
      },
      {
        ...
        Resource = "arn:aws:ssm:${data.aws_region.current.name}:${data.aws_caller_identity.current.account_id}:parameter${var.routes_ssm_path}*"
      },
    ]
  })
}
...
resource "aws_security_group" "ecs" {
  ...
  description = "Controls access to the shared mTLS proxy Fargate service"
  vpc_id      = var.vpc_id
...
resource "aws_ecs_task_definition" "main" {
  ...
  container_definitions = jsonencode([{
    ...
    environment = [
      { name = "SSM_ROUTES_PATH",                value = var.routes_ssm_path },
      { name = "REGION",                         value = data.aws_region.current.name },
      { name = "SSM_TRANSPORT_KEY_NAME",         value = aws_ssm_parameter.server_tls_key.name },
      { name = "SSM_TRANSPORT_CERTIFICATE_NAME", value = aws_ssm_parameter.server_tls_certificate.name },
...
resource "aws_route53_record" "proxy" {
  name    = "${var.external_hostname}.${var.external_domain_name}"
  type    = "CNAME"
